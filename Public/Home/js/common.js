...
function showWindow(k, url, mode, cache, menuv,obj) {
	...
	var fetchContent = function() {
		if(mode == 'get') {
			...
			menuObj.act = $(url).action;
                        ajaxpost(url, 'fwin_content_' + k, '', '', '', function() {initMenu();show();if(typeof obj !='undefined'){switchjiracon(obj.pid);}});
		}
...
function switchjiracon(pid){
	if(pid>0){
		var postsubject = $('h_postsubject_'+pid).innerHTML;
		var postreurl = $('h_reporturl_'+pid).innerHTML;
		var postreporter = $('h_reporter_'+pid).innerHTML;
		$('jiradescription').value = trim($('postmessage_'+pid).innerHTML)+'\r\n\r\n\r\n原始地址:'+postreurl+'\r\n\r\n'+postreporter;
		$('threadpid').value = pid;
		if(postsubject!=''){
			$('summary').value = postsubject;
		}
	}
}
...
function simulateSelect(selectId, widthvalue,fn_call) {
	...
	for(var i = 0; i < selectObj.options.length; i++) {
		...
		if(selectObj.options[i].selected) {
			...
			selectObj.setAttribute('selecti', i);
                ...
                if(selectObj.options[i].getAttribute('did')){
                    li.setAttribute('title',selectObj.options[i].getAttribute('did'));
                }
		li.onclick = function() {
                        if(typeof fn_call == 'function'){fn_call(selectId,this.getAttribute('title'),this.k_value);}
...
function MIUI_initSearchmenu(searchform, cloudSearchUrl) {
	var defaultUrl = 'search.php?searchsubmit=yes';
	if(typeof cloudSearchUrl == "undefined" || cloudSearchUrl == null || cloudSearchUrl == '') {
		cloudSearchUrl = defaultUrl;
	}
...
function cardInit() {
	var cardShow = function (obj) {
		...
		USERCARDST = setTimeout(function() {ajaxmenu(obj, 0, 1, 2, pos, null, 'p_pop card');}, 250);
	};
	var cardids = {};
