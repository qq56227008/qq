	...
	if(tmph > baseh) {
		...
		octrl.onclick = function () {
			if(this.className == cls[0]) {
				...
				setcookie('auto_hide_thread_types', 0, 2592000);
			} else {
				o.style.height = '';
				this.className = cls[0];
				this.innerHTML = lang[0];
				setcookie('auto_hide_thread_types', 1, 2592000);
			}
		}
	}
}
...
function fixed_top_nv(eleid, disbind) {
	...
	this.run = function () {
		...
		if(this.openflag && this.nvdata.next){
			...
			if(dofixed) {
				if(this.nv.style.position != 'fixed') {
					...
					/*MIUI_CHANGE START 下拉滚动页面时，导航栏变成通栏*/
					$('nv_fixed').style.display='block';
					$('nv_fixed').appendChild(this.nv);
		...
		if(this.nv) {
			...
			/*MIUI_CHANGE START 下拉滚动页面时，导航栏变成通栏*/			
			$('hd_wp').appendChild(this.nv);
			$('nv_fixed').style.display='none';
			//$('hd').style.background = "url("+src+") 0 116px repeat-x #f9f9f9";
			$('nv').style.background = ' 0 116px #FD6440';
			/*MIUI_CHANGE END*/
		}
	};
...
function previewThread(tid, tbody) {
	if(!$('threadPreviewTR_'+tid)) {
		...
		previewTid = tid;
                ...
                $('thread_th_'+tid).style.borderBottom='1px solid #e7e7e7';
		...
		$('thread_th_'+tid).style.borderBottomColor = 'transparent';
	} else {
                $('thread_th_'+tid).style.borderBottom='1px dotted #CDCDCD';
		$(tbody).removeChild($('threadPreviewTR_'+tid));
		previewTbody = previewTid = null;
		$('thread_th_'+tid).style.borderBottomColor = '#cdcdcd';
	}
}
