...
function setpmstatus(form) {
	...
	for(var i = 0; i < checkboxs.length; i++) {
		var e = checkboxs[i];
		if(e.id && e.id== 'checkbox_'+e.value) {			
			if(e.name.match('deletepm_g')) {
				ids_gpmid.push(e.value);
				markreadids.push('gpmlist_' + e.value);
			} else if(e.name.match('deletepm_delplid') || e.name.match('deletepm_q')) {
				ids_plid.push(e.value);
				markreadids.push('pmlist_' + e.value);
			}
		}else if(e.id && e.id.match('checkbox_'+e.value+'_')){
                                         var tv = e.id.split('_');
                                         if(tv[2]>0){
                                            ids_plid.push(tv[2]);
                                            markreadids.push('pmlist_' + tv[2]);
                                        }
                                }
	}
	/*MIUI CHANGE END*/
...
function addBlockLink(id, tag) {
	...
	var taglist = {'A':1, 'INPUT':1, 'IMG':1, 'SPAN':1};
